...
(
    ...
    `current_num`  INT UNSIGNED     DEFAULT 1     NOT NULL COMMENT '当前分块编号(从1开始)',
    `current_size` BIGINT UNSIGNED  DEFAULT 0     NOT NULL COMMENT '当前分块实际大小(Byte)',
    `chunk_size`   BIGINT UNSIGNED  DEFAULT 0     NOT NULL COMMENT '分块大小(Byte)',
    `total_size`   BIGINT UNSIGNED                NOT NULL COMMENT '文件总大小(Byte)',
    `total_chunks` INT UNSIGNED                   NOT NULL COMMENT '文件总块数(Byte)',
